...
👉 项目演示地址: https://github.com/jilanlan/vue-myMusic/tree/master/dist （*在 GitHub Pages 中，通过 jsonp 请求的数据会被正常渲染，而由于无 node 服务，通过 axios 请求的数据不会被正常渲染*）

基于 **Vue (2.x)** 制作的移动端音乐 WebApp 。
...
- `vuex`：Vue 集中状态管理，在多个组件需要共享某些状态时非常便捷
...
1. 歌曲数据全部来自 QQ 音乐，接口改变了可能就要修改 `jsonp` 和 `axios` 代码，部分数据是采用vue的devSrever代理后端请求
2. 在请求单个歌曲的源文件时，要动态传入id以及vkey值，vkey也是代理请求的，但请求过来和官网的值不同，百度得知qq音乐的源文件的vkey值一直是变化的，所以在开发播放器内核页时遇到问题，有大神解决请赐教。
...
复用歌单详情页，在基础组件中动态传入值控制排行图标的显示与隐藏
