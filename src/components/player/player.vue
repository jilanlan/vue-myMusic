          ...
          <scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p ref="lyricLine" class="text" v-for="(line,index) in currentLyric.lines" :key="line.index" :class="{'current':currentLineNum === index }">{{line.txt}}</p>
              </div>
            </div>
          </scroll>
        </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      updateTime(e) {
        this.currentTime = e.taget.currentTime
      },
      format(interval) {
        interval = interval | 0
    ...
    watch: {
      currentSong(newSong, oldSong) {
        ...
        this.timer = setTimeout(() => {
          this.$refs.audio.play()
          this.getLyric()
          // 由于歌曲的源文件链接官方保护了 这里拿不到 所以播放不了 后面需要在歌曲成功加载之后再做下一曲和上一曲的动作，封装了ready和error函数，这里也无法封装了
        }, 1000)
      },
      playing(newPlaying) {
